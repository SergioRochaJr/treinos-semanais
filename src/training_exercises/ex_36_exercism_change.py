...
def coin_change(coins: list[int], change: int) -> list[int]:
    """Return the fewest coins needed to make the given change amount.

    Uses a dynamic programming approach to compute the minimum number of coins
    required to reach the target amount. The returned list contains a valid
    combination of coin values that sums to the requested change, sorted in
    ascending order.

    Args:
        coins: A list of available coin values.
        change: The target amount of change to make.

    Returns:
        A sorted list of coin values that add up to the target amount.

    Raises:
        ValueError: If the requested change is negative or cannot be made with
            the provided coins.
    """
    if change < 0:
